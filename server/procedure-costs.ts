...
const procedureCostsByDoctorOld: Record<string, ProcedureCostData[]> = {
  ...
  'icb-transplante': [
    { name: 'Transplante Capilar', cost: 2000 },
    { name: 'Body Hair', cost: 2000 }
  ],
  'dr-basile': [
    { name: 'Mastopexia com Protese', cost: 1210 },
    { name: 'Mastopexia', cost: 1210 },
...
// Custos NOVOS dos procedimentos (válidos a partir de 01/10/2024)
...
// Função para obter o custo de um procedimento específico para um médico em uma data específica
export function getProcedureCost(doctor: string, procedureName: string, entryDate?: string): number {
  // Determinar qual tabela de preços usar baseado na data
  let costsTable: Record<string, ProcedureCostData[]>;
  
  if (entryDate) {
    const date = new Date(entryDate);
    // Se a data é anterior a 01/10/2024, usar preços antigos
    costsTable = date < PRICE_CHANGE_DATE ? procedureCostsByDoctorOld : procedureCostsByDoctor;
  } else {
    // Se não tem data, usar preços novos (padrão)
    costsTable = procedureCostsByDoctor;
  }
  
  const doctorCosts = costsTable[doctor];
...
export function calculateProcedureCosts(procedures: Array<{ procedure: string; count: number; }>, doctor: string, entryDate?: string): number {
  return procedures.reduce((total, proc) => {
    const procedureCost = getProcedureCost(doctor, proc.procedure, entryDate);
